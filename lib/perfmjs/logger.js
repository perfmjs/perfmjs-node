    ...
    perfmjs.utils.extend(perfmjs.logger.prototype, {
        ...
        dateFileAppend: function (dirName, fileName, msg) {
            ...
            if (arguments.length == 2) {
                ...
                msg = arguments[1];
                dirName = arr.slice(0, arr.length - 1).join('/');
                fileName = arr[arr.length - 1];
            }
            var path = dirName + '/' + this._formatDate(new Date).substring(0, 10) + '-' + fileName;
            var fs = require('fs');
        ...
        _handler: function (level, name, msg) {
            ...
            msg = ([['[' + this._formatDate(new Date()) + "] [" + method + "] [default] - "].join("-")].concat(Array.prototype.slice.call(msg))).join('');
            if (console.log.apply) {
                console[method].apply(console, [msg]);
            }
            //写入date file
            this.dateFileAppend(perfmjs.sysConfig.config.get('appenderFile') || process.cwd() + '/logs/app.log', msg);
        },
