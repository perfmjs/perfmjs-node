...
perfmjs.plugin('app', function($$) {
	$$.base("app", {
        ...
        register: function(creator, opt) {
            try {
                ...
                if(!DEBUG_MOD){
                    perfmjs.logger.error("could not register " + moduleId + " because: " + ex.stack);
                }else{
                    throw ex;
                }
                return undefined;
		...
		*/
		start: function(moduleId){
			...
			try {
				...
				//lib.logger.debug(moduleId + " init finished, cost:" + ($$.utils.now() - start) + " ms");
				return true;
			} catch (ex) {
				if(!DEBUG_MOD){
					perfmjs.logger.error(moduleId + " init Error: " + ex.stack);
				}else{
					throw ex;
				}
			  return false;
    ...
    if (typeof module !== 'undefined' && module.exports) {
        exports = module.exports = perfmjs.app;
    }
    /*for Node.js end*/
});