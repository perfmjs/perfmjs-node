/**
 * perfmjs(high performance javascript framework) for Node.js  V1.1.8
 * @date 2014-05-19
 */
!(function(global) {
    //"use strict";
    var perfmjs = this, _namespace = function() {
        ...
        for (; i < a.length; i++) {
            ...
            if (arg.indexOf('.')) {
                d = arg.split('.');
                for (j = (d[0] === 'perfmjs') ? 1 : 0; j < d.length; j++) {
                    o[d[j]] = o[d[j]] || {};
                    o = o[d[j]];
                }
            } else {
    ...
    perfmjs.utils = {
        ...
        aop: function(origFn, before, after, context) {
            var self = this;
            return function() {
                var args, result;
                ...
                if (before && typeof before === 'function') {
                    ...
                    //如果返回非undefined的结果, 则停止执行原方法
                    if (typeof args !== 'undefined') {
                        return args;
                    }
                }
                //如果before返回一个数组，则用来替换原有参数
                args = self.isArray(args) ? args : arguments;
                //调用原对象的原方法
                result = origFn.apply(context, args);
    ...
    /**
     * perfmsj-node框架的app启动函数，调用例子如下：
       //应用入口函数
       var jsbfPusher = require("./lib/push/server/jsbf/jsbfPusher");
    ...
    if (perfmjs.utils.isNodeJSSupport()) {
        exports = module.exports = perfmjs;
        perfmjs.utils.forEach(['./sys-config', './base', './logger', './joquery',
                               './event-proxy', './fsm', './app', './profiler'], function(item, index) {
            require(item);
        });
    }
    /*for Node.js end*/
